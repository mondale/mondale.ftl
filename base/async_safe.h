...
// Writes `len` bytes from `buf` to `fd`, retrying short writes and EINTR; returns true on success.
bool WriteAll(int fd, const char* buf, size_t len);

// Enumerates thread IDs by reading /proc/self/task, storing up to `max_tids` into `tids` and returning the count.
int EnumerateThreads(int* tids, int max_tids);

// Parses a hexadecimal number (digits [0-9a-fA-F]) from *p up to `end`, advancing *p on success.
// Returns false if no hex digits were present.
bool ParseHex(const char** p, const char* end, uint64_t* out);

// Buffered ASCII writer that accumulates output and flushes it to the provided file descriptor.
class Writer final {
 public:

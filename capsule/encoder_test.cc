...
TEST_F(EncoderTest, EncodeOneU32) {
  ...
  EXPECT_EQ(8 + 1 * 8, len) << "Inner header plus one field.";
...
TEST_F(EncoderTest, EncodeOneString) {
  ...
  EXPECT_EQ(8 + 1 * 8 + (4 + 13 + 7) / 8 * 8, len)
      << "Inner header, field, 4-length encoding, length so string rounded up "
         "a multiple of 8.";
...
TEST_F(EncoderTest, EncodeNestedCapsule) {
  ...
  auto* const inner_inner_header =
      reinterpret_cast<capsule::abi::InnerHeader*>(&capsule1_.space[0]);
  EXPECT_EQ(1, inner_inner_header->offset_table_count);
  EXPECT_EQ(16, inner_inner_header->capsule_length);
  auto* const inner_ot =
